...
public class MoveToTrailUVEditor : Editor
{
    ...
    {
        // m_overrideMaterial_sp = serializedObject.FindProperty("m_overrideMaterial");
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            string message = String.Format("Element {0} Change Trail Renderer Texture Mode to Tile.", checkTrailRenderertile);
        ...
        {
            // m_overrideMaterial_sp.boolValue = EditorGUILayout.ToggleLeft(new GUIContent("Override Material", "UV Tiling 값을 어디에서 수정할지 방향을 결정합니다. 체크를 켜면 Move To Trail UV 컴포넌트의 UV Tiling 값이 우선하게 되고, 체크를 끄면 재질의 UV Tiling 값이 우선하게 됩니다."), m_overrideMaterial_sp.boolValue); // ToggleLeft를 사용하기 위해 boolValue 사용
    ...
    private void InitializeEditor()
    {
        ...
        {
            SerializedProperty materialDataElement_sp = m_materialData_sp.GetArrayElementAtIndex(i);
            SyncElementTiling(materialDataElement_sp); // Tiling
    ...
    private string CheckTrailRendererShader()
    {
        ...
        for (int i = 0; i < m_mttuv.m_materialData.Length; i++)
        {
            ...
            if (mat == null)
            {
                string message = String.Format("Element {0}missing material.", i);
                return message;
            }
